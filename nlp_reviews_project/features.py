"""features.py
Compute linguistic features for each review using spaCy + NLTK/VADER.
"""
import pandas as pd
import numpy as np
from collections import Counter
from preprocessing import tokenize_lemmatize, get_nlp, clean_text
import textstat
import nltk
...
def pos_ratios(text):
    ...
    return {
        ...
        'noun_ratio': (counts.get('NOUN', 0) + counts.get('PROPN', 0)) / total,
...
def superlative_count(text):
    nlp = get_nlp()
    doc = nlp(text)
    return sum(1 for tok in doc if tok.tag_ in ('JJS','RBS'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', default='nlp_reviews_project/data/feature_reviews.csv')
